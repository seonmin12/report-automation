...
(더미데이터로 바로 체험 가능, 업로드 파일은 DB나 영구 저장소에 저장되지 않고 검증 처리에만 임시 사용됩니다)
...
- 요금제/상품 미매핑 현황 점검
- 사업자별 누적 가입자 집계 (월간 운영 리포트 시나리오)
- 실적 요약 이메일 공지 (고정 문구 + 요약 이미지·엑셀 첨부)

## 프로젝트 의의

이 프로젝트는 단순히 Excel 리포트를 자동 생성하는 도구가 아니라, 반복적인 실적 검증·공지 업무를
자동화 가능한 단위로 분리해 구현한 업무자동화 PoC입니다.

실무에서 반복적으로 발생하는 파일 다운로드, 데이터 대사, 오류 확인, 리포트 작성, 메일 초안 작성
흐름을 더미데이터 기반으로 재구성했습니다. 특히 자동화 결과를 그대로 확정하지 않고, 오류상세
리포트와 요약 결과를 제공하여 사람이 최종 확인할 수 있도록 설계했습니다.

이를 통해 데이터 정합성 검증, 예외 케이스 탐지, 반복 리포트 생성, 메일 초안 작성처럼 실무 운영에서
자주 발생하는 업무를 Python/FastAPI 기반 자동화 파이프라인으로 구현했습니다.
...
(`web/app.py`의 `CORSMiddleware`). 로컬 개발 및 데모 환경에서는 인증/쿠키를 사용하지 않는
read-only API이므로 CORS를 넓게 허용했습니다. 실제 운영 환경에서는 허용 오리진을 배포
도메인으로 제한하는 방식이 필요합니다.
...
- 사용자 계정/권한 관리
- DB 기반 검증 이력 저장
- Object Storage 기반 결과물 보관
- 승인/검토 워크플로우
- 메일 발송 시스템 연동은 보안·권한·오발송 리스크 검토 후 제한적으로 확장 가능
  (OAuth 동의화면·앱 검증·토큰 저장이 필요해, 로그인/DB 없는 현재 구조에서는 의도적으로 제외)
...
```text
제목: [MVNO 운영팀] 7월 19일 마감 실적 공유드립니다
...
| 월간 운영 리포트(사업자별 누적가입자) | 당월 누적 집계 (aggregate.py → 사업자별누적가입자 시트) |
